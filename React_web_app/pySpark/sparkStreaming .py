...
"""
#gcloud dataproc jobs submit pyspark --cluster hw2 sparkStreaming.py --region us-east1
...
# global variables
bucket = "stream3"    # TODO : replace with your own bucket name
...
output_dataset = 'stream'                     #the name of your dataset in BigQuery
...
def saveToBigQuery(sc, output_dataset, output_table, directory):
    ...
    subprocess.check_call(
        ...
        '--replace '
        '--autodetect '
        '{dataset}.{table} {files}'.format(dataset=output_dataset, table=output_table, files=files).split())
...
def hashtagCount(words):
    ...
    hashtags = words.filter(lambda w: w.startswith('#') and len(w)>=2).map(lambda x: (x.lower(), 1))
    tags_totals = hashtags.updateStateByKey(updateFunc)
    sorted = tags_totals.transform(lambda rdd: rdd.sortBy(lambda pair: -pair[1]))
    return sorted


def updateFunc(new_values, last_sum):
    return int(math.fsum(new_values) + (last_sum or 0))
...
def wordCount(words):
    ...
    count = words.filter(lambda w: 'ai' == w.lower() or 'data' == w.lower() or 'movie' == w.lower() or 'spark' == w.lower() or 'good' == w.lower()).map(lambda x: (x.lower(), 1))
    count_total = count.reduceByKeyAndWindow(lambda x, y: x + y, lambda x, y: x - y, 60, 60).transform(lambda rdd: rdd.map(lambda x: (x[0], x[1], datetime.now().strftime('%Y-%m-%d %H:%M:%S'))))
